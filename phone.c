...
void namefun(){
	...
	double phone;
	char gender[8];
    fptr = fopen("ebookvn.txt", "ab+"); // ab+ gives us the ability of writing the function and add the second data in the existing one...
    if(fptr==NULL){
...
void modifyfun(){
	...
	fclose(fptr);
	fclose(fptr1);
    fptr = fopen("ebookvn.txt", "w");
    fclose(fptr);
    fptr = fopen("ebookvn.txt", "a");
...
void exitfun(){
	...
	gotoxy(31,6);
	printf("\xDB VIVEKANAND.");
	gotoxy(31,8);
	
}
void password(void){
	char passwords[20]={"SORRY"};
	gotoxy(22,2);
	int j;
	int z;
